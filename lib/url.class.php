<?php
  ...
  class Url
  {
       ...
       * @return $string
       */
	  public static function crumbsfrontend($url, $pointer, $home, $is_page = false)
	  {
		  ...
		  if ($only == 1) {
			//   $breadcrumbs = '<div class="active section">' . $home . '</div>';
			  ...
			  foreach ($url as $key => $part) {
				...
				//   if (is_array($part)) {
				// 	  if ($key == $last_key) {
				// 		  $breadcrumbs .= '<h3 style="text-align:center;color:#fff">' . ucwords($part[0]) . '</h3>';
				// 	  } else {
				// 		  $breadcrumbs .= '<a href="' . SITEURL . '/' . substr($last_segment, 1) . '/" class="section">' . ucwords(str_replace($url[0], $home, $part[0])) . '</a>';
				// 	  }
				//   } else {
					  if ($key == $last_key) {
						  $breadcrumbs .= '<h3 style="text-align:center;color:#fff">' . ucwords($part) . '</h3>';
					  } else {
						  $breadcrumbs .= '<a href="' . SITEURL . '/' . substr($last_segment, 1) . '/" class="section">' . ucwords(str_replace($url[0], $home, $part)) . '</a>';
					  }
				//   }
