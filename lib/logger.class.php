<?php
   ...
   * @copyright 2020
   * @version $Id: logger.class.php, v1.00 2020-04-20 18:20:24 gewa Exp $
   */
  ...
  class Logger
  {
      ...
      private static function setRecord($failed_last)
      {
          ...
          if ($row = self::getRecord()) {
              Db::run()->pdoQuery("UPDATE `" . self::lTable . "` SET failed_last = " . $failed_last . ", failed = failed + 1 WHERE id = ?;", array($row->id));
          } else {
              $data = array(
                  'ip' => Url::getIP(),
                  'type' => "user",
