<?php
  ...
  class Content
  {
      ...
      public function Pages()
      {
          ...
          $pager->items_total = (App::Auth()->usertype <> "owner") ? Db::run()->count(false, false, "SELECT COUNT(*) FROM `" . self::pTable . "` WHERE `is_admin` = 0 LIMIT 1") : Db::run()->count(self::pTable);
          $pager->default_ipp = App::Core()->perpage;
          $pager->path = Url::url(Router::$path, "?");
          $pager->paginate();

          $where = (App::Auth()->usertype <> "owner") ? array("is_admin" => 0) : null;
	  ...
	  public function processMenu()
	  {
		  ...
		  foreach (App::Core()->langlist as $lang) {
			  $rules['name_' . $lang->abbr] = array('required|string|min_len,2|max_len,80', Lang::$word->MEN_NAME . ' <span class="flag icon ' . $lang->abbr . '"></span>');
	  ...
	  public function copyPage()
	  {
		  ...
		  if (empty(Message::$msgs)) {
			  ...
			  $rows = Db::Run()->first(self::pTable, null, array("id" => Filter::$id));
			  
			  foreach(new LimitIterator(new ArrayIterator($rows), 1) as $key => $row) {
				  $data[$key] = (isset($row)) ? $row: "";
			  }

			  $last_id = Db::run()->insert(self::pTable, $data)->getLastInsertId();
			  Db::run()->update(self::pTable, $datam, array("id" => $last_id));
	  ...
	  public function processCountry()
	  {
		  ...
		  if (empty(Message::$msgs)) {
			  ...
			  if ($data['home'] == 1) {
				  Db::run()->pdoQuery("UPDATE `" . self::cTable . "` SET `home`= ?;", array(0));
			  }	
      ...
      public function Templates()
      {
		  ...
		  $tpl->data = Db::run()->select(self::eTable, null, null, "ORDER BY name" . Lang::$lang)->results(); 
	  ...
	  public static function rendertCustomFields($id, $section)
	  {
		  ...
		  if ($data) {
			  foreach ($data as $i => $row) {
				  ...
				  $html .= '<label>' . $row->{'title' . Lang::$lang} . $required . $tootltip . '</label>';
				  $html .= '</div>';
	   ...
	   * @return
	   */
	  public static function rendertCustomFieldsFront($id, $section, $type = null)
	  {
		  ...
		  $html .= Utility::getSnippets(THEMEBASE . "/snippets/customFields.tpl.php", $data = ["data" => $result, "id" => $id, "section" => $section, "type" => $type]);
	  ...
	  public function processCoupon()
	  {
		  ...
		  $rules = array(
			  ...
			  'discount' => array('required|numeric|min_numeric,1|max_numeric,100', Lang::$word->DC_DISC),
      ...
      public static function pageHeading()
	  {
		  ...
		  } elseif(File::is_File(UPLOADS . '/images/hongkong.jpg')) {
			  $bg = ' style="background-image: url(' . UPLOADURL . '/images/hongkong.jpg);"';
		  } else {
			  $bg = '';
		  }
