<?php
  ...
  switch ($delete):
		  ...
		  case "deleteMenu":
              ...
              $json['message'] = $title . ' - ' . Lang::$word->DELETED;
			  print json_encode($json);
		  break; 
      ...
      case "deleteModule":
          ...
          /* == Delete Plugin == */
      case "deletePlugin":
          if ($row = Db::run()->delete(Plugins::mTable, array("id" => Filter::$id))):
              $json['type'] = "success";
          endif;

          $json['title'] = Lang::$word->SUCCESS;
          $json['message'] = str_replace("[NAME]", $title, Lang::$word->PLG_DEL_OK);
          print json_encode($json);
          break;
  ...
  switch ($restore):
	  ...
	  case "restoreMenu":
		  if($result = Db::run()->first(Core::txTable, array('dataset'), array("id" => filter::$id))):
			  ...
			  Message::msgReply(true, 'success', $title . ' - ' . Lang::$word->RESFRTR);
		  endif;
	  break;
